#!/bin/bash
# FranceClips Clips
if [ -d "$MEDIA_DIR${DPG_DIR}" ]; then
    for file in "$MEDIA_DIR${DPG_DIR}"/**/*.mp4; do
        ...
        echo "### Tweet Clipfile ###"
        #twurl tweet -d "media_ids=$MEDIA_ID&status=​$DPG_EMOJI DiaperPoop Girl 🎥 $DL_LINK $TWITTER_DPG_HTAGS"
    done
fi