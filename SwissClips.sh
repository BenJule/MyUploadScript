...
# SwissClips Clips
if [ -d "$MEDIA_DIR" ]; then
    for file in "$MEDIA_DIR${CH_DIR}"/**/*.mp4; do
        ...
        TUMBLRDATA=$(echo "$DATA" | jq -r '.result[] | "client.create_photo((blogName), state=\"'"$TUMBLR_STATE"'\", tags=[\"'"$TUMBLR_CH_HTAGS"'\"], format=\"html\", source=\"" + .single_img + "\", " + " caption=\"'"$CH_EMOJI"'\" <h2>" + .title + "</h2><br><br> 🎥 <a href=" + .protected_dl + ">Watch Clip</a> 🎥 <br><br> 🐦 <a href='https://twitter.com/USER'> Follow me on Twitter </a> \") " ')
        ...
        TWITTERDATA=$(echo "$DATA" | jq -r '.result[] | " twurl tweet -d '"$CH_EMOJI"' " + .title + " 🎥 " + .protected_dl + " '"$TWITTER_CH_HTAGS"' " ')
        echo "$TWITTERDATA" | tee >>./Twitter.txt

        echo "### Tweet Clipfile ###"
        #twurl tweet -d "media_ids=$MEDIA_ID&status=​$CH_EMOJI Schweiz 🎥 $DL_LINK $TWITTER_CH_HTAGS"
    done
fi
